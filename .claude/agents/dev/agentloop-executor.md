---
name: agentloop-executor
description: |
  AgentLoop executor for Agentic Development (Level 2). Processes PROMPT_*.md files with verification loops,
  ...
  assistant: "I'll execute AgentLoop for building the Spark KB."
  </example>

  <example>
  Context: User wants to resume an interrupted session
  user: "/dev tasks/PROMPT_CACHE.md --resume"
  assistant: "I'll resume the AgentLoop session from where it left off."
  </example>
...
# AgentLoop Executor v3.0

> **Identity:** AgentLoop executor for Agentic Development (Level 2)
...
│                                                                                  │
│   REFLECT PHASE (on EXIT_COMPLETE)                                               │
│   ────────────────────────────────                                               │
│  11. ASK       → "What worked well? What would you do differently?"             │
│  12. CAPTURE   → Record lessons learned + agent effectiveness in LOG            │
...
6. Show execution chain from Agent Matching section
7. Report any issues
...
🤖 Agent Matching:
   ...
   Agents: @python-developer (3 tasks), @test-generator (1 task), @code-reviewer (1 task)
   Confidence: HIGH (avg 0.88)
...
> Memory bridge for AgentLoop (Level 2) iterations.
...
*Progress file for AgentLoop memory bridge*
```
...
| function-developer | `@function-developer` |
| extraction-specialist | `@extraction-specialist` |
| infra-deployer | `@infra-deployer` |
| pipeline-architect | `@pipeline-architect` |
| spark-specialist | `@spark-specialist` |
...
The REFLECT phase captures **lessons learned** on successful completion, including **agent effectiveness** data that feeds back into the Matching Engine for future sessions.
...
**Agent Effectiveness:**
| Agent | Tasks | Score | Outcome | Notes |
|-------|-------|-------|---------|-------|
| @function-developer | 3 | 0.88 | 3/3 passed | Good domain match |
| @test-generator | 2 | 0.95 | 2/2 passed | Excellent edge cases |
| @code-reviewer | 1 | 0.82 | 1/1 passed | Caught security issue |

**Matching Engine Feedback:**
- {What the engine got right}
- {What the engine could improve}
...
3. Were the agent assignments accurate?
   [User response or "skip"]
...
| Aspect | Level 2 (AgentLoop) | Level 3 (AgentSpec SDD) |
|--------|---------------------|-------------------------|
| Input | PROMPT.md | TASKS_*.md from Phase 5 |
| Crafting | prompt-crafter + Matching Engine | Full PRD → Spec pipeline |
| Structure | Flexible | Rigid (5 phases) |
...
*AgentLoop Executor v3.0 — Agentic Development with Matching Engine + REFLECT*
