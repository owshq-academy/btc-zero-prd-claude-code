...
- **LLM Stack:** Gemini 2.0 Flash (Vertex AI) → OpenRouter (Claude/GPT-4o) fallback
...
- [ ] **Task 2.7:** Implement three-layer Validator
  ...
  - **Acceptance:** Validates JSON → Pydantic → Business rules
...
- [ ] **Task 2.9:** Implement main Extractor orchestrator
  - **Agent:** @python-developer
  - **Description:** Create `src/invoice-extractor/extractor.py` to orchestrate full pipeline: ImageProcessor → Classifier → PromptManager → LLMGateway → Validator → Aggregator
...
|---|-----------|---------------------|--------|
| 1 | All dependencies install cleanly | `pip install -r src/invoice-extractor/requirements.txt` | ⬜ |
| 2 | Project imports without errors | `python -c "from src.invoice_extractor import extractor"` | ⬜ |
| 3 | Schema validation passes | `pytest tests/unit/test_schema.py -v` | ⬜ |
| 4 | Image processing works | `pytest tests/unit/test_image_processor.py -v` | ⬜ |
| 5 | LLM gateway with fallback works | `pytest tests/integration/test_llm_gateway.py -v` | ⬜ |
| 6 | Three-layer validation passes | `pytest tests/unit/test_validator.py -v` | ⬜ |
| 7 | End-to-end extraction works | `pytest tests/integration/test_extractor.py -v` | ⬜ |
| 8 | Parquet output is valid | `python -c "import pyarrow.parquet as pq; pq.read_table('data/output/test.parquet')"` | ⬜ |
| 9 | CLI command works | `python -m src.invoice_extractor.cli extract data/input/ubereats_INV-UE-19561B_20260123.tiff --dry-run` | ⬜ |
| 10 | All unit tests pass | `pytest tests/unit/ -v` | ⬜ |
| 11 | All integration tests pass | `pytest tests/integration/ -v -m "not live"` | ⬜ |
| 12 | Code passes type checking | `mypy src/invoice-extractor/ --strict` | ⬜ |
| 13 | Code passes linting | `ruff check src/invoice-extractor/` | ⬜ |
| 14 | Smoke test passes | `bash scripts/smoke_test.sh` | ⬜ |
| 15 | Sample files extracted >= 90% accuracy | Manual validation with ground truth | ⬜ |
...
- **Design Document:** design/invoice-extractor-design.md
- **Requirements:** design/invoice-extractor-requirements.md
- **Architecture:** 5-stage pipeline (Image → Classifier → Prompt → LLM → Validator)
- **LLM Stack:** Gemini 2.0 Flash (primary) → OpenRouter (fallback)
- **Validation:** 3-layer (JSON parse → Pydantic schema → Business rules)
